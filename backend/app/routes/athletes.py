...
def get_today_workout(
    ...
    workout = db.query(models.Workout).filter(
        and_(
            models.Workout.athlete_id == current_athlete.id,
            models.Workout.scheduled_date >= today,
            models.Workout.scheduled_date < datetime(today.year, today.month, today.day, 23, 59, 59)
        )
    ).first()
...
def start_workout(
    ...
    if workout.athlete_id != current_athlete.id:
        raise HTTPException(
